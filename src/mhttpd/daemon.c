...
#define EXTRA_SLOTS 0
  ...
  while ( (MHD_YES != con->daemon->shutdown) && (MHD_CONNECTION_CLOSED != con->state) )
    {
      ...
      if (0 == (con->daemon->options & MHD_USE_POLL))
	{
      ...
      {
        if (MHD_YES !=add_to_fd_set (spipe, &rs, &maxsock, FD_SETSIZE))
          err_state = 1;
      }
      #endif
      if (0 != err_state)
  ...
  if (MHD_CONNECTION_IN_CLEANUP != con->state)
    {
      ...
      if (MHD_CONNECTION_CLOSED != con->state)
	      MHD_connection_close_ (con, MHD_REQUEST_TERMINATED_DAEMON_SHUTDOWN);  //状态变为MHD_CONNECTION_CLOSED 
      con->idle_handler (con);// connection_handle_idle() 里面处理超时
    }
exit:   // close conn 上面的while循环检查到 conn状态为CLOSED 时候退出
  ...
  if (MHD_INVALID_SOCKET != con->socket_fd)// 清除 response， shutdown(WR) close(fd)
    {
...
#endif
      return MHD_NO;
    }// 如果 NULL != daemon->worker_pool 后面的代码不再 运行
...
//end MHD_WINSOCK_SOCKETS
  ...
  if (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION))
  {
   ...
   XDLL_insert (daemon->normal_timeout_head,
                ...
                connection);  //mark question epoll?
  DLL_insert (daemon->connections_head,
	      daemon->connections_tail,
	      connection);   //插入到daemon中
  if  ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION)) &&
 ...
 */
static void
MHD_cleanup_connections (struct MHD_Daemon *daemon) //清楚cleanup的连接队列，并调用 MHD_mutex_unlock_ 关闭fd
{
  struct MHD_Connection *pos;
  ...
  while (NULL != (pos = daemon->cleanup_head))
    {
      DLL_remove (daemon->cleanup_head,
		  daemon->cleanup_tail,
		  pos);
      if ( (0 != (daemon->options & MHD_USE_THREAD_PER_CONNECTION)) && (MHD_NO == pos->thread_joined) )
	   {
		     ...
		     const fd_set *except_fd_set)
{
  ...
  if (0 == (daemon->options & MHD_USE_THREAD_PER_CONNECTION))  //如果不是 MHD_USE_THREAD_PER_CONNECTION 直接在当前线程处理
    {
      /* do not have a thread per connection, process all connections now */
      next = daemon->connections_head; //处理一个daemon 中所有的连接 
      while (NULL != (pos = next))
      {
	      next = pos->next;
        ds = pos->socket_fd;
        if (MHD_INVALID_SOCKET == ds)
	        continue;
        call_handlers (pos, FD_ISSET (ds, read_fd_set), FD_ISSET (ds, write_fd_set), MHD_NO); //里面会处理超时
      }
    }
	    ...
	    int may_block)
{
  ...
  if (0 == (daemon->options & MHD_USE_THREAD_PER_CONNECTION))
    {
      ...
      if (MHD_NO == MHD_get_fdset2 (daemon, &rs, &ws, &es, &maxsock, FD_SETSIZE))// 获取 damon 所有conn 的fd
        {
          #ifdef HAVE_MESSAGES
  ...
  else
    {
      ...
      if ( (MHD_INVALID_SOCKET != daemon->socket_fd) &&
           (MHD_YES != add_to_fd_set (daemon->socket_fd,
                                      &rs,
                                      &maxsock,
                                      FD_SETSIZE)) ) //只处理server socket的fd
        {
          #ifdef HAVE_MESSAGES
  ...
  if ( (MHD_INVALID_PIPE_ != daemon->wpipe[0]) &&
                                  ...
                                  FD_SETSIZE)) )
    {
      ...
      #ifdef HAVE_MESSAGES
      MHD_DLOG (daemon, "Could not add control pipe FD to fdset");
      #endif
      err_state = MHD_YES;
    }
		     ...
		     va_list ap)
{
  ...
  if (use_pipe)
  {
    ...
    if (MHD_NO == make_nonblocking (daemon, daemon->wpipe[0]))
      {
#ifdef HAVE_MESSAGES
        MHD_DLOG (daemon, "Failed to make control pipe non-blocking: %s\n", MHD_strerror_ (errno));
#endif
  ...
  if (MHD_YES != parse_options_va (daemon, &servaddr, ap))
    {
#if HTTPS_SUPPORT
      if ( (0 != (flags & MHD_USE_SSL)) && (NULL != daemon->priority_cache) )
	      gnutls_priority_deinit (daemon->priority_cache);
#endif
      free (daemon);
      return NULL;
  ...
  if ( (0 == (flags & MHD_USE_SELECT_INTERNALLY)) &&
       (daemon->worker_pool_size > 0) )  //MHD_USE_SELECT_INTERNALLY 时候才使用  thread pool
    {
#ifdef HAVE_MESSAGES
  ...
  if (0 != (flags & (MHD_USE_SELECT_INTERNALLY | MHD_USE_THREAD_PER_CONNECTION)))
    {
#ifdef HAVE_MESSAGES
      MHD_DLOG (daemon, "Threaded operations are not supported on Symbian.\n");
#endif
      goto free_and_fail;
    }
#endif
  if ( (MHD_INVALID_SOCKET == daemon->socket_fd) &&
       (0 == (daemon->options & MHD_USE_NO_LISTEN_SOCKET)) )
    {
      /* try to open listen socket */
      if (0 != (flags & MHD_USE_IPv6))
	      socket_fd = create_listen_socket (daemon, PF_INET6, SOCK_STREAM, 0);
      else
	      socket_fd = create_listen_socket (daemon, PF_INET, SOCK_STREAM, 0);
      ...
      if (0 == daemon->listening_address_reuse)
        {
          ...
          if (0 > setsockopt (socket_fd, SOL_SOCKET, SO_REUSEADDR, (void*)&on, sizeof (on)))
          {
            #ifdef HAVE_MESSAGES
...
#define SO_REUSEPORT 15
...
#endif
#ifdef SO_REUSEPORT
          if (0 > setsockopt (socket_fd, SOL_SOCKET, SO_REUSEPORT, (void*)&on, sizeof (on)))
            {
#ifdef HAVE_MESSAGES
              MHD_DLOG (daemon, "setsockopt failed: %s\n", MHD_socket_last_strerr_ ());
#endif
...
#ifdef HAVE_MESSAGES
          MHD_DLOG (daemon, "Cannot allow listening address reuse: SO_REUSEPORT not defined\n");
#endif
          goto free_and_fail;
#endif
#endif
      ...
      else /* if (daemon->listening_address_reuse < 0) */
        {
          ...
          if (0 > setsockopt (socket_fd, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (void*)&on, sizeof (on)))
            {
#ifdef HAVE_MESSAGES
              MHD_DLOG (daemon, "setsockopt failed: %s\n", MHD_socket_last_strerr_ ());
#endif
...
#ifdef HAVE_MESSAGES
          MHD_DLOG (daemon, "Cannot disallow listening address reuse: SO_EXCLUSIVEADDRUSE not defined\n");
#endif
          goto free_and_fail;
#endif
#endif /* _WIN32 */
...
#endif
  if ( ( (0 != (flags & MHD_USE_THREAD_PER_CONNECTION)) ||  // 1. MHD_USE_THREAD_PER_CONNECTION
	 ( (0 != (flags & MHD_USE_SELECT_INTERNALLY)) &&           //2. MHD_USE_SELECT_INTERNALLY && worker_pool_size == 0
	   (0 == daemon->worker_pool_size)) ) &&
       (0 == (daemon->options & MHD_USE_NO_LISTEN_SOCKET)) &&  //  &&  !MHD_USE_NO_LISTEN_SOCKET
       (0 != (res_thread_create =
       ...
       (0 == (daemon->options & MHD_USE_NO_LISTEN_SOCKET)) )
    {
	  ...
	  if ( (0 != (daemon->options & MHD_USE_EPOLL_LINUX_ONLY)) &&
	    ...
	    goto thread_failed;
          ...
          if (0 != (res_thread_create =
		    create_thread (&d->pid, daemon, &MHD_select_thread, d)))  //启动线程池
            {
#ifdef HAVE_MESSAGES
              MHD_DLOG (daemon,
