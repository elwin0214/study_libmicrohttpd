  ...
  do
    {
      ...
      if ( (MHD_YES == connection->have_chunked_upload) &&
           (MHD_SIZE_UNKNOWN == connection->remaining_upload_size) )
        {
          if ( (connection->current_chunk_offset == connection->current_chunk_size) &&
               (0 != connection->current_chunk_offset) &&  //开始一个新的chunk
               (available >= 2) )
            {
          ...
          if (connection->current_chunk_offset <
              connection->current_chunk_size)
            {
              ...
              processed =
                connection->current_chunk_size -  // 如果是chunk 每个chunk至少调用一次 default_handler
                connection->current_chunk_offset;
              if (processed > available)  // 只能获取到部分当前chunk数据
                processed = available;
              if (available > processed)// 有比当前chunk更多的数据可用
          ...
          else
            {
              ...
              if (available > 0)  //有更多的数据，需要继续while处理
                instant_retry = MHD_YES;
              if (0 == connection->current_chunk_size)  // chunk 结束
                {
      ...
      if (0 != processed)// 没有处理完直接退出while ? 如果传递的是一个完整的chunk，但是default_handler 没有处理。
        instant_retry = MHD_NO; /* client did not process everything */
      used -= processed;
      if (connection->have_chunked_upload == MHD_YES)
        connection->current_chunk_offset += used; // current_chunk_offset 表示 当前已处理的数据大小
      /* dh left "processed" bytes in buffer for next time... */
      buffer_head += used;
      available -= used;
      if (connection->remaining_upload_size != MHD_SIZE_UNKNOWN)
        connection->remaining_upload_size -= used; // 根据 content-length 还需要处理多少数据
    }
...
MHD_connection_handle_read (struct MHD_Connection *connection)
{
  ...
  if (MHD_CONNECTION_CLOSED == connection->state)// 下面的do_read 在遇到异常或者client关闭,设置状态为MHD_CONNECTION_CLOSED
    return MHD_YES;
  ...
  while (1)
    {
      ...
      switch (connection->state)
        ...
        case MHD_CONNECTION_FOOTER_PART_RECEIVED:
          ...
          break;
        case MHD_CONNECTION_CLOSED://进入while之前 state 已经是 CLOSED 直接退出
          return MHD_YES;
        default:
  ...
  while (1)
    {
      ...
      switch (connection->state)
        ...
        case MHD_CONNECTION_HEADERS_PROCESSED:
		 ...
		 (MHD_str_equal_caseless_ (connection->method,
				   MHD_HTTP_METHOD_PUT))) )  // 遇到 post 或者 put 并且response已经设置的情况下，关闭conn
            {
        ...
        case MHD_CONNECTION_CONTINUE_SENDING:
          ...
          break;
        case MHD_CONNECTION_CONTINUE_SENT:// 接收数据
          if (0 != connection->read_buffer_offset)
            {
          ...
          if ((0 == connection->remaining_upload_size) ||
            ...
            {
              if ((MHD_YES == connection->have_chunked_upload) &&
                  (MHD_NO == connection->read_closed))  //chunk client 关闭写入
                connection->state = MHD_CONNECTION_BODY_RECEIVED;
              else
                connection->state = MHD_CONNECTION_FOOTERS_RECEIVED;
              continue;
            }
          break;  // 退出 switch while 去读数据
        case MHD_CONNECTION_BODY_RECEIVED:
          line = get_next_header_line (connection, NULL);
          if (NULL == line)
            {
        ...
        case MHD_CONNECTION_FOOTER_PART_RECEIVED:
          ...
          continue;
        case MHD_CONNECTION_FOOTERS_RECEIVED:  //  GET request go here
          call_connection_handler (connection); /* "final" call */
          if (connection->state == MHD_CONNECTION_CLOSED)
            continue;
          if (NULL == connection->response)  // 如果 call_connection_handler 是空操作 一直循环下去？
        ...
        case MHD_CONNECTION_NORMAL_BODY_UNREADY:
          ...
          if (0 == connection->response->total_size)  //total_size  表示输出的response的大小，不论是buffer还是 callback 都是预先确定的
            {
              if (NULL != connection->response->crc)
          ...
          if (MHD_YES == try_ready_normal_body (connection))
            {
	      if (NULL != connection->response->crc)
	        (void) MHD_mutex_unlock_ (&connection->response->mutex);//end if
              connection->state = MHD_CONNECTION_NORMAL_BODY_READY;// 设置为ready 在外部的 handle_write 中调用 do_write 写数据
              /* Buffering for flushable socket was already enabled*/  //同时 check_write_done() 如果发现writebuffer 没有写完 会设置为 unready 写完会设置为 sent
              if (MHD_NO == socket_flush_possible (connection))
                socket_start_no_buffering (connection);
